...
class JWTAuth:
    ...
    def __encode_jwt(
        self, data: dict, validity_period: int = VALIDITY_PERIOD
    ) -> Tuple[bool, str]:
        """
        生成 jwt 认证信息
            @param:
        ...
        try:
            ...
            jwt_body = jwt.encode(payload, self.secret_key, algorithm="HS256")
            return True, jwt_body
        except Exception as e:
            logger.exception(e)
        return False, ""

    def decode_jwt(self, jwt_str: str, verify_exp: bool = True) -> Tuple[bool, dict]:
        """
        解析 jwt 认证信息
            @param:
                jwt_body: jwt 字串
    ...
    def decode_jwt_check_refresh_jwt(
        self, jwt_str: str, refresh_jwt_str: str
    ) -> Tuple[bool, dict]:
    ...
    def create_jwt_and_refresh_jwt(
        self, data: dict, validity_period: int = VALIDITY_PERIOD
    ) -> Tuple[bool, str, str]:
        """
        ...
        # 生成 jwt
        for _ in range(5):
            jwt_stat, jwt = self.__encode_jwt(data, validity_period=validity_period)
            if jwt_stat is True:
                break
            else:
                continue
        ...
        for _ in range(5):
            re_jwt_stat, refresh_jwt = self.__encode_jwt(
                {"v": refresh_data}, validity_period=validity_period * 2
            )
