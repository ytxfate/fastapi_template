...
async def handle_request_validation_error(
    ...
    return comm_ret(
        code=resp_code.PARAMETER_ERROR, msg="\n".join(msg) or "请求参数异常", resp=resp
    )
...
async def handle_http_exception(request: Request, exc: StarletteHTTPException):
    ...
    if isinstance(exc, RateLimitExceeded):
        return comm_ret(code=resp_code.API_LIMIT, isSuccess=False, msg="限流")
    # 用户认证
    # 此处添加异常校验的原因是 project/dependencies/auth_depend.py 中自定义的
    # check_jwt 函数只能通过 raise 异常的方式返回结果
    if exc.status_code in [
        resp_code.JWT_PARSE_ERROR,
        resp_code.USER_NO_AUTHORITY,
        resp_code.USER_NO_LOGIN,
    ]:
        return comm_ret(code=exc.status_code, msg=exc.detail)

    return comm_ret(
        code=resp_code.EXCEPTION_ERROR, isSuccess=False, msg="HTTP Exception"
    )
...
async def handle_all_exception(request: Request, exc: Exception):
    logger.error(exc)
    return comm_ret(
        code=resp_code.EXCEPTION_ERROR, isSuccess=False, msg="System Exception"
    )
