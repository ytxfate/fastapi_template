...
def _check(
    ...
    """token 检验"""
    if not jwt:
        raise HTTPException(status_code=resp_code.USER_NO_LOGIN, detail="用户未登录")
    # 解析 jwt 信息
    if jwt_stat == "NOT":
    ...
    if decode_status is False or not user_info:
        raise HTTPException(
            status_code=resp_code.JWT_PARSE_ERROR, detail="刷新用户令牌中..."
        )
    try:
        ...
        logger.exception(e)
        raise HTTPException(
            status_code=resp_code.USER_NO_LOGIN, detail="用户信息解析异常,请重新登录"
        )
    ...
    if security_scopes.scopes:  # 判断是否需要检验 scopes
        ...
        if set(user_scopes) >= set(security_scopes.scopes):
            ...
            raise HTTPException(
                status_code=resp_code.USER_NO_AUTHORITY, detail="用户没有此接口的权限"
            )
    else:
        return jwtbi
